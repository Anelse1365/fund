...
<?php
    ...
    if(isset($_SESSION["user"]) && $_SESSION["usertype"] === 'p') {
        ...
        // ตรวจสอบว่ามีผลลัพธ์หรือไม่ก่อนที่จะดึงข้อมูล
        if($result->num_rows > 0) {
            $userfetch = $result->fetch_assoc();
            $userid = $userfetch["pid"];
            $username = $userfetch["pname"];
        } else {
            // ถ้าไม่มีผู้ใช้งานในฐานข้อมูล ให้ทำอะไรต่อเช่นให้แสดงข้อความผิดพลาดหรืออื่นๆ
            // เช่น header("Location: ../error.php");
            exit("ไม่พบข้อมูลผู้ใช้");
        }
    } else {
        // ถ้าไม่มี session หรือไม่ใช่ผู้ใช้ประเภท 'p' ให้เปลี่ยนเส้นทางไปยังหน้า login
